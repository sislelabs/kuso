...
func installKind() {
	...
	kf, _ := installer.R().Get("/sislelabs/kuso/main/cli/kind.yaml")
	...
	// set cluster name
	kindConfig.Name = promptLine("Kind Cluster Name", "", "kuso-"+strconv.Itoa(rand.Intn(1000)))

	// select Kubernetes version
	kv, _ := installer.R().Get("/sislelabs/kuso/main/cli/templates/kindVersions.yaml")
