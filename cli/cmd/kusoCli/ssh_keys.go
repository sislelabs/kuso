...
var sshKeyRmYes bool

var sshKeyRmCmd = &cobra.Command{
	Use:     "rm <id>",
	Aliases: []string{"remove", "delete"},
	Short:   "Delete a stored SSH key by id.",
	Long: "Delete a stored SSH key by id.\n\n" +
		"If the key was generated by kuso (--generate) the private key exists\n" +
		"nowhere else and cannot be recovered; node join/validate flows that\n" +
		"reference it stop working immediately.",
	Args: cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		if api == nil {
			return fmt.Errorf("not logged in; run 'kuso login' first")
		}
		if err := confirmDestructive(sshKeyRmYes,
			fmt.Sprintf("Delete SSH key %s? A kuso-generated private key exists nowhere else and cannot be recovered.", args[0])); err != nil {
			return err
		}
...
func init() {
	...
	sshKeyRmCmd.Flags().BoolVarP(&sshKeyRmYes, "yes", "y", false, "skip the confirmation prompt")
