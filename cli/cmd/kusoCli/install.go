...
var installCmd = &cobra.Command{
	Use:   "install",
	Short: "Create a Kubernetes cluster and install all required components for kuso",
	Long: `This command will create a kubernetes cluster and install all required components 
for kuso on any kubernetes cluster.
...
func init() {
	installCmd.Flags().StringVarP(&argComponent, "component", "c", "", "install component (kubernetes,olm,ingress,metrics,certManager,kuso-operator,monitoring,kuso-ui)")
	installCmd.Flags().StringVarP(&argAdminUser, "user", "u", "", "Admin username for the kuso UI")
	...
	installCmd.Flags().StringVarP(&argPort, "port", "p", "", "Kuso UI HTTP port")
	installCmd.Flags().StringVarP(&argPortSecure, "securePort", "P", "", "Kuso UI HTTPS port")
	installCmd.Flags().StringVarP(&argDomain, "domain", "d", "", "Domain name for the kuso UI")
...
func printInstallSteps() {
  ...
  Steps to install kuso:
    ...
    8. Install the kuso UI {{(optional, but highly recommended)}}::gray
    9. Write the kuso CLI config {{(optional)}}::gray
`)
}
...
func tellAChucknorrisJoke() {
	...
	jokesApi := resty.New().
		...
		SetHeader("User-Agent", "kuso-cli/"+kusoCliVersion).
...
func checkCluster() {
	...
	clusterSelect := promptLine("Is the CURRENT cluster the one you wish to install Kuso?", "[y,n]", "y")
	if clusterSelect == "n" {
		os.Exit(0)
	}
}
...
func installMetrics() {
	...
	components := "https://raw.githubusercontent.com/sislelabs/kuso/main/operator/deploy/metrics-server.yaml"
...
func installKusoOperator() {

	_, _ = cfmt.Println("\n  {{3) Install Kuso Operator}}::bold")

	kusoInstalled, _ := exec.Command("kubectl", "get", "operator", "kuso-operator.operators").Output()
	if len(kusoInstalled) > 0 {
		_, _ = cfmt.Println("{{✓ Kuso Operator is already installed}}::lightGreen")
		return
	}

	if installOlm {
		installKusoOLMOperator()
	} else {
		installKusoOperatorSlim()
	}
}

func installKusoOLMOperator() {

	kusoSpinner := spinner.New("Install Kuso Operator")
	_, _ = cfmt.Println("  run command : kubectl apply -f https://operatorhub.io/install/kuso-operator.yaml")
	kusoSpinner.Start("Install Kuso Operator")
	_, kusoErr := exec.Command("kubectl", "apply", "-f", "https://operatorhub.io/install/kuso-operator.yaml").Output()
	if kusoErr != nil {
		fmt.Println("")
		kusoSpinner.Error("Failed to run command to install the Operator. Try running this command manually: kubectl apply -f https://operatorhub.io/install/kuso-operator.yaml")
		log.Fatal(kusoErr)
	}

	kusoSpinner.UpdateMessage("Wait for Kuso Operator to be ready")
	var kusoWait []byte
	for len(kusoWait) == 0 {
		// kubectl api-resources --api-group=application.kuso.sislelabs.com --no-headers=true
		kusoWait, _ = exec.Command("kubectl", "api-resources", "--api-group=application.kuso.sislelabs.com", "--no-headers=true").Output()
		time.Sleep(1 * time.Second)
	}

	kusoSpinner.Success("Kuso Operator installed successfully")

}

func installKusoOperatorSlim() {

	kusoSpinner := spinner.New("Install Kuso Operator")
	_, _ = cfmt.Println("  run command : kubectl apply -f https://raw.githubusercontent.com/sislelabs/kuso/main/operator/deploy/operator.yaml")
	kusoSpinner.Start("Install Kuso Operator")
	_, kusoErr := exec.Command("kubectl", "apply", "-f", "https://raw.githubusercontent.com/sislelabs/kuso/main/operator/deploy/operator.yaml").Output()
	if kusoErr != nil {
		fmt.Println("")
		kusoSpinner.Error("Failed to run command to install the Operator. Try running this command manually: kubectl apply -f https://raw.githubusercontent.com/sislelabs/kuso/main/operator/deploy/operator.yaml")
		log.Fatal(kusoErr)
	}

	kusoSpinner.UpdateMessage("Wait for Kuso Operator CRD's to be installed")
	var kusoWait []byte
	for len(kusoWait) == 0 {
		// kubectl api-resources --api-group=application.kuso.sislelabs.com --no-headers=true
		kusoWait, _ = exec.Command("kubectl", "api-resources", "--api-group=application.kuso.sislelabs.com", "--no-headers=true").Output()
	...
	kusoSpinner.UpdateMessage("Kuso Operator CRD's installed")

	time.Sleep(5 * time.Second)
	// kubectl wait --for=condition=available deployment/kuso -n kuso --timeout=180s
	kusoSpinner.UpdateMessage("Wait for Kuso Operator to be ready")
	_, olmWaitErr := exec.Command("kubectl", "wait", "--for=condition=available", "deployment/kuso-operator-controller-manager", "-n", "kuso-operator-system", "--timeout=300s").Output()
	if olmWaitErr != nil {
		kusoSpinner.Error("Failed to wait for Kuso UI to become ready")
		log.Fatal(olmWaitErr)
	}
	kusoSpinner.Success("Kuso Operator installed successfully")
...
func createNamespace(namespace string) {
	...
	kusoNSInstalled, _ := exec.Command("kubectl", "get", "ns", namespace).Output()
	if len(kusoNSInstalled) > 0 {
		_, _ = cfmt.Printf("{{✓ Namespace %s exists}}::lightGreen\n", namespace)
	} else {
		_, kusoNSErr := exec.Command("kubectl", "create", "namespace", namespace).Output()
		if kusoNSErr != nil {
...
func installKusoUi() {
	...
	ingressInstall := promptLine("8) Install Kuso UI", "[y,n]", "y")
	if ingressInstall != "y" {
		return
	}

	createNamespace("kuso")

	gitops := promptLine("Enable GitOps Feature", "[y,n]", "n")

	kusoSecretInstalled, _ := exec.Command("kubectl", "get", "secret", "kuso-secrets", "-n", "kuso").Output()
	if len(kusoSecretInstalled) > 0 {
		_, _ = cfmt.Println("{{✓ Kuso Secret exists}}::lightGreen")
	} else {
		...
		*/
		...
		createSecretCommand := exec.Command("kubectl", "create", "secret", "generic", "kuso-secrets",
			"--from-literal=KUSO_WEBHOOK_SECRET="+webhookSecret,
			"--from-literal=KUSO_SESSION_KEY="+sessionKey,
			"--from-literal=KUSO_ADMIN_USERNAME="+argAdminUser,
			"--from-literal=KUSO_ADMIN_PASSWORD="+argAdminPassword,
			//"--from-literal=KUSO_USERS="+userDBencoded, // DEPRECATED in v3.0.0
		)
		...
		createSecretCommand.Args = append(createSecretCommand.Args, "-n", "kuso")

		_, kusoErr := createSecretCommand.Output()

		if kusoErr != nil {
			_, _ = cfmt.Println("{{✗ Failed to run command to create the secrets.}}::red")
			log.Fatal(kusoErr)
		} else {
			_, _ = cfmt.Println("{{✓ Kuso Secret created}}::lightGreen")
		}
	}

	kusoUIInstalled, _ := exec.Command("kubectl", "get", "kusoes.application.kuso.sislelabs.com", "-n", "kuso").Output()
	if len(kusoUIInstalled) > 0 {
		_, _ = cfmt.Println("{{✓ Kuso UI already installed}}::lightGreen")
	} else {
		installer := resty.New()
		...
		if argDomain == "" {
			argDomain = promptLine("Kuso UI Domain", "", "kuso.localhost")
		}
		kusoUIConfig.Spec.Ingress.Hosts[0].Host = argDomain

		// Warn if domain contains kuso.net

		webhookDomain := argDomain
		if strings.Contains(argDomain, "kuso.localhost") {
			_, _ = cfmt.Println("{{⚠ kuso.localhost might not be reachable won't get any Webhooks. GitHub will fail to connect your pipeline. }}::yellow")
			webhookDomain = "webhook.local.kuso.net"
		}
		...
		kusoUIConfig.Spec.Kuso.WebhookURL = webhookURL

		kusoUISsl := promptLine("Enable SSL for the Kuso UI", "[y/n]", "y")
		if kusoUISsl == "y" {

			clusterIssuer := promptLine("Kuso UI ClusterIssuer", "", "letsencrypt-prod")
			kusoUIConfig.Spec.Ingress.Annotations.KubernetesIoIngressClass = clusterIssuer
			kusoUIConfig.Spec.Ingress.Annotations.KubernetesIoTlsAcme = "true"

			kusoUIConfig.Spec.Ingress.TLS = []KusoUITls{
				{
		...
		if gitops == "y" {
			...
			kusoUIRegistry := promptLine("Enable Buildpipeline for Kuso (BETA)", "[y/n]", "n")
			if kusoUIRegistry == "y" {
				kusoUIConfig.Spec.Registry.Enabled = true

				kusoUICreateRegistry := promptLine("Create a local Registry for Kuso", "[y/n]", "n")
				if kusoUICreateRegistry == "y" {
					kusoUIConfig.Spec.Registry.Create = true

					kusoUIRegistryStorage := promptLine("Registry storage size", "", "10Gi")
					kusoUIConfig.Spec.Registry.Storage = kusoUIRegistryStorage

					storageClassList := getAvailableStorageClasses()

					kusoUIRegistryStorageClassName := selectFromList("Registry storage class", storageClassList, "")
					kusoUIConfig.Spec.Registry.StorageClassName = kusoUIRegistryStorageClassName
				}

				kusoUIRegistryHost := promptLine("Registry", "[registry.kuso.mydomain.com]", "")
				kusoUIConfig.Spec.Registry.Host = kusoUIRegistryHost

				kusoUIRegistrySubPath := promptLine("Subpath (optional) ", "[example/foo/bar]", "")
				kusoUIConfig.Spec.Registry.SubPath = kusoUIRegistrySubPath

				kusoUIConfig.Spec.Registry.Port = 443

				kusoUIRegistryUsername := promptLine("Registry username", "", "admin")
				kusoUIConfig.Spec.Registry.Account.Username = kusoUIRegistryUsername

				kusoUIRegistryPassword := promptLine("Registry password", "", generateRandomString(12, ""))
				kusoUIConfig.Spec.Registry.Account.Password = kusoUIRegistryPassword

				kusoUIRegistryPasswordBytes, _ := bcrypt.GenerateFromPassword([]byte(kusoUIRegistryPassword), 14)
				kusoUIConfig.Spec.Registry.Account.Hash = string(kusoUIRegistryPasswordBytes)
			}
		}

		storageClassList := getAvailableStorageClasses()
		kusoUIDatabaseStorageClassName := selectFromList("Database storage class", storageClassList, "")
		kusoUIConfig.Spec.Kuso.DataBase.StorageClassName = kusoUIDatabaseStorageClassName

		kusoUIDatabaseStorage := promptLine("Database storage size", "", "10Gi")
		kusoUIConfig.Spec.Kuso.DataBase.Size = kusoUIDatabaseStorage

		if monitoringInstalled {
			kusoUIConfig.Spec.Prometheus.Enabled = true
			kusoUIConfig.Spec.Prometheus.Endpoint = promptLine("Prometheus URL", "", "http://kuso-prometheus-server")
		} else {
			kusoUIConfig.Spec.Prometheus.Enabled = false
		}

		kusoUIConsole := promptLine("Enable Console Access to running containers", "[y/n]", "y")

		if kusoUIConsole == "y" {
			kusoUIConfig.Spec.Kuso.Config.Kuso.Console.Enabled = true
		}

		// DEBUG
		//kusoUIConfig.Spec.Image.Tag = "v3.1.1-rc.1"
		...
		if clusterType == "linode" ||
			...
			clusterType == "gke" {
			kusoUIConfig.Spec.Ingress.ClassName = "nginx"
		}

		kusoUIYaml, _ := yaml.Marshal(kusoUIConfig)
		kusoUIErr := os.WriteFile("kusoUI.yaml", kusoUIYaml, 0644)

		if kusoUIErr != nil {
			fmt.Println(kusoUIErr)
			return
		}

		_, olmInstallErr := exec.Command("kubectl", "apply", "-f", "kusoUI.yaml", "-n", "kuso").Output()
		if olmInstallErr != nil {
			_, _ = cfmt.Println("{{✗ Failed to run command to install Kuso UI. Try running this command manually: kubectl apply -f kusoUI.yaml -n kuso}}::red")
			return
		} else {
			e := os.Remove("kusoUI.yaml")
			if e != nil {
				log.Fatal(e)
			}
			_, _ = cfmt.Println("{{✓ Kuso UI installed}}::lightGreen")
		}

		kusoUISpinner := spinner.New("Wait for Kuso UI to be created")
		kusoUISpinner.Start("Wait for Kuso UI to be created")

		var kusoWait []byte
		for len(kusoWait) == 0 {
			// kubectl get --ignore-not-found deployment kuso
			kusoWait, _ = exec.Command("kubectl", "get", "--ignore-not-found", "deployment", "kuso", "-n", "kuso").Output()
			kusoUISpinner.UpdateMessage("Waiting for Kuso UI to be created")
			time.Sleep(1 * time.Second)
		}

		kusoUISpinner.UpdateMessage("Waiting for Kuso UI to be ready")

		time.Sleep(5 * time.Second)
		// kubectl wait --for=condition=available deployment/kuso -n kuso --timeout=180s
		_, olmWaitErr := exec.Command("kubectl", "wait", "--for=condition=available", "deployment/kuso", "-n", "kuso", "--timeout=300s").Output()
		if olmWaitErr != nil {
			kusoUISpinner.Error("Failed to wait for Kuso UI to become ready")
			log.Fatal(olmWaitErr)
		}
		kusoUISpinner.Success("Kuso UI is ready")
	}
...
func installMonitoring() {
	...
	if promptLine("7.1) Create local Prometheus instance", "[y/n]", "y") == "y" {
		URL := "https://raw.githubusercontent.com/sislelabs/kuso/main/operator/config/samples/application_v1alpha1_kusoprometheus.yaml"
		_, _ = cfmt.Println("  run command : kubectl apply -n kuso -f " + URL)
		spinnerObj.Start("Installing Prometheus")
		_, ingressErr := exec.Command("kubectl", "apply", "-n", "kuso", "-f", URL).Output()
		if ingressErr != nil {
		...
		/*
			...
			time.Sleep(5 * time.Second)
			// kubectl wait --for=condition=available deployment/kuso -n kuso --timeout=180s
			_, olmWaitErr := exec.Command("kubectl", "wait", "--for=condition=available", "deployment/kuso-prometheus-server", "-n", "kuso", "--timeout=300s").Output()
			if olmWaitErr != nil {
	...
	if promptLine("7.2) Enable KubeMetrics", "[y/n]", "y") == "y" {
		_, _ = cfmt.Println("  run command : kubectl patch kusoes kuso -n kuso --type=merge")
		...
		_, patchResult := exec.Command("kubectl", "patch", "kusoprometheuses", "kuso-prometheus", "-n", "kuso", "--type=merge", "-p", patch).Output()
		if patchResult != nil {
			spinnerObj.Error("Failed to patch the kuso prometheus CRD to enable kube metrics", patchResult.Error(), patch)
		}
...
func installCertManagerClusterIssuer(namespace string) {
	...
	certManagerClusterIssuerYamlErr := os.WriteFile("kusoCertManagerClusterIssuer.yaml", certManagerClusterIssuerYaml, 0644)
	if certManagerClusterIssuerYamlErr != nil {
		fmt.Println(certManagerClusterIssuerYamlErr)
		return
	}

	_, certManagerClusterIssuerErr := exec.Command("kubectl", "apply", "-f", "kusoCertManagerClusterIssuer.yaml", "-n", namespace).Output()
	if certManagerClusterIssuerErr != nil {
		_, _ = cfmt.Println("{{✗ Failed to create CertManager ClusterIssuer. Try running this command manually: kubectl apply -f kusoCertManagerClusterIssuer.yaml -n cert-manager}}::red")
		return
	} else {
		e := os.Remove("kusoCertManagerClusterIssuer.yaml")
		if e != nil {
			log.Fatal(e)
		}
...
func writeCLIConfig() {
	...
	ingressInstall := promptLine("9) Write the Kuso CLI config", "[y,n]", "n")
	if ingressInstall != "y" {
		return
	}

	//TODO consider using SSL here.
	url := promptLine("Kuso Host address", "", "http://"+argDomain+":"+argPort)
	viper.Set("api.url", url)

	token := promptLine("Kuso Token", "", argApiToken)
...
func printDNSinfo() {
	...
	ingressInstalled, err := exec.Command("kubectl", "get", "ingress", "-n", "kuso", "-o", "json").Output()
	if err != nil {
		_, _ = cfmt.Println("{{✗ Failed to fetch DNS information}}::red")
		return
	}
	var kusoIngress KusoIngress
	_ = json.Unmarshal(ingressInstalled, &kusoIngress)
	...
	if len(kusoIngress.Items) > 0 &&
		len(kusoIngress.Items[0].Spec.Rules[0].Host) > 0 &&
		len(kusoIngress.Items[0].Status.LoadBalancer.Ingress) > 0 &&
		len(kusoIngress.Items[0].Status.LoadBalancer.Ingress[0].IP) > 0 {
		_, _ = cfmt.Printf("{{  %s.		IN		A		%s}}::lightBlue\n", kusoIngress.Items[0].Spec.Rules[0].Host, kusoIngress.Items[0].Status.LoadBalancer.Ingress[0].IP)
		_, _ = cfmt.Printf("{{  *.review.example.com.			IN		A		%s}}::lightBlue", kusoIngress.Items[0].Status.LoadBalancer.Ingress[0].IP)
	}
...
func finalMessage() {
	...
	_, _ = cfmt.Println("\n\n    {{Done - you can now login to your Kuso UI}}::lightGreen\n\n ")
