...
func appForm() kusoApi.AppCRD {

	var appCRD kusoApi.AppCRD

	pipelineConfig := loadPipelineConfig(pipelineName, false)

	appCRD.APIVersion = "application.kuso.sislelabs.com/v1alpha1"
	appCRD.Kind = "KusoApp"
...
func writeAppYaml(appCRD kusoApi.AppCRD) {
	// write pipeline.yaml
	...
	fileName := ".kuso/" + appCRD.Spec.Pipeline + "/" + appCRD.Spec.Phase + "/" + appCRD.Spec.Name + ".yaml"
