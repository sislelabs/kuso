...
func installLinode() {
	...
	_, _ = cfmt.Println("{{⚠ Installing Kubernetes on Linode is currently beta state in kuso-cli}}::yellow")
	...
	clusterConfig.Label = promptLine("Cluster Name", "", "kuso-"+strconv.Itoa(rand.Intn(1000)))
	...
	clusterConfig.Tags = []string{"kuso"}
