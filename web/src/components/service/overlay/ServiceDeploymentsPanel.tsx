...
type Status = "succeeded" | "failed" | "running" | "pending" | "unknown";

function classify(b: BuildSummary): Status {
...
function statusBadge(s: Status) {
  const map: Record<Status, { label: string; cls: string }> = {
    succeeded: { label: "ACTIVE", cls: "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" },
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center rounded px-1.5 py-0.5 font-mono text-[9px] font-semibold tracking-widest border",
        m.cls
      )}
    >
      {m.label}
            ...
            return (
              <li
                ...
                className={cn(
                  ...
                  s === "succeeded" && isFirst && "border-emerald-500/30",
                  !((s === "failed") || (s === "succeeded" && isFirst)) && "border-[var(--border-subtle)]"
                )}
              >
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[10px] font-mono text-[var(--text-tertiary)]">{created}</div>
                  </div>
                  {isOpen ? (
