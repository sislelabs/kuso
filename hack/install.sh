...
#   KUSO_SERVER_VERSION  server image tag (default: v0.3.0; a Go
#                        binary published at
...
KUSO_SERVER_VERSION="${KUSO_SERVER_VERSION:-v0.3.0}"
...
curl -sfL "${KUSO_RAW}/deploy/server-go.yaml" \
  | sed "s|kuso-server-go:v0.3.0|kuso-server-go:${KUSO_SERVER_VERSION}|g" \
  | kubectl apply -f - >/dev/null
