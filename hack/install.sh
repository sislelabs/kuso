...
#   KUSO_SERVER_VERSION  server image tag (default: v0.2.0-rc7; a Go
#                        binary published at
...
KUSO_SERVER_VERSION="${KUSO_SERVER_VERSION:-v0.2.0-rc7}"
...
curl -sfL "${KUSO_RAW}/deploy/server-go.yaml" \
  | sed "s|kuso-server-go:v0.2.0-rc7|kuso-server-go:${KUSO_SERVER_VERSION}|g" \
  | kubectl apply -f - >/dev/null
