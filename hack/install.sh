...
#   KUSO_VERSION         operator image tag (default: v0.2.0-multitenant)
#   KUSO_SERVER_VERSION  server image tag (default: v0.2.0-rc11; a Go
#                        binary published at
...
KUSO_VERSION="${KUSO_VERSION:-v0.2.0-multitenant}"
KUSO_SERVER_VERSION="${KUSO_SERVER_VERSION:-v0.2.0-rc11}"
...
curl -sfL "${KUSO_RAW}/deploy/server-go.yaml" \
  | sed "s|kuso-server-go:v0.2.0-rc11|kuso-server-go:${KUSO_SERVER_VERSION}|g" \
  | kubectl apply -f - >/dev/null
