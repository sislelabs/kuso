...
func (d *Dispatcher) onPullRequest(ctx context.Context, body []byte) error {
	...
	for _, proj := range projects {
		...
		case "opened", "reopened", "synchronize":
			...
			//
			// Gate on previewDeployed > 0: if no service matched the PR's
			// repo (or all had previews disabled / failed to deploy), there
			// is NO preview env, so posting a "Preview ready for review"
			// comment with a reviewer URL is a lie — the link would open a
			// review page for a preview that doesn't exist. Only advertise
			// a reviewer surface when at least one preview actually came up.
...
_Auto-generated by kuso %s preview reviewer._`, reviewURL, version.Version())
