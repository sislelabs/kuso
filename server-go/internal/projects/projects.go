...
type Service struct {
	...
	// (Removed: describeCache. Describe's three list calls all go
	// through the cached list[T] helper in kube/crds.go, which is
	// informer-served. The 5s TTL only saved every third poll on a
	// 15s cadence while requiring an explicit invalidateDescribe()
	// contract at ~10 mutator sites — a manual invariant that's
	// easy to leak when adding new mutators. invalidateDescribe()
	// stays as a no-op so call sites compile; it can be deleted
	// once the codebase has been swept.)
