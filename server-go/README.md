...
**Status:** Phase 0 — scaffold only. The TS server in `server/` remains the
production backend; do not deploy this image yet.

## Layout

```
server-go/
├── cmd/kuso-server/    main.go: flag parsing, signal handling, http.Server
├── internal/
