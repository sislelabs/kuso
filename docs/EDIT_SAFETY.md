...
| `sleep` | ⚠️ Yes, with caveat | Scale-down watcher pauses pods after `afterMinutes` of zero traffic; the first request hits the activator, which **holds the live connection and proxies the original request through** once a pod is Ready (default budget 60s, `KUSO_ACTIVATOR_HOLD_SECONDS`). So a woken request — including a webhook POST body — is delivered, not dropped, *as long as cold-start fits the hold budget*. The residual risk is a **cold image pull** on a node that doesn't have the image: if wake exceeds the budget the activator returns 503+Retry-After and a webhook sender that doesn't retry loses that event. For services taking third-party webhooks/payment callbacks (Stripe, ePay, GitHub, Slack, etc.), use **`sleep.wakeOn.excludePaths`** to keep the deployment warm (forces min-1 replica and excludes it from scale-down — whole-deployment, not per-path routing). Sleep with no excludePaths is safe for backoffice tools, internal dashboards, and preview environments. **Note:** `sleep` is a service-level field; it propagates identically to every environment (there is no per-env override), but the scale-down watcher only ever slates the `-production` env. |
...
| `buildArgs` | ✅ Yes, on next build | Non-secret build-time constants (v0.23.1+): flow to `--build-arg` (dockerfile) / build-time `ENV` (nixpacks). No effect on running pods — takes effect only when the next build runs, so trigger a rebuild to apply. Plain literals only; `secretKeyRef` values are dropped server-side (never persisted in a layer). Not for secrets — use a release hook or a `--mount=type=secret` RUN. |
| `snapshotBeforeDeploy` | ✅ Yes, no restart | Only affects future deploys. When true AND a release hook exists, the poller snapshots subscribed postgres addons (a normal manifested backup, `trigger=pre_deploy`) before the migration runs; on migration failure the build is annotated with the snapshot keys for a one-click restore. No effect on running pods; no data impact from toggling. Snapshot infra-failure blocks that deploy. |
